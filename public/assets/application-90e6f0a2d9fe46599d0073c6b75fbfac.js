  ...
  $(function() {
    return $('.btn-group .btn').click(function() {
      ...
      if ($(this).hasClass('btn-holiday')) {
        ...
        return $(document).find('#movie-week').hide();
      } else {
        $(document).find('#holiday-week').hide();
        return $(document).find('#movie-week').show();
      }
    });
  });

  toggleButtonGroup = function(btn) {
    var group;
    group = btn.parent();
...
(function() {
  $(function() {
    return $('#message_nature').change(function() {
      var form, phone, selected;
      ...
      if (selected === 'Select One...') {
        phone.hide();
        return form.hide();
      } else if (selected === 'Booking/General Enquiry') {
        phone.show();
        return form.hide();
      } else {
        phone.hide();
...
(function() {
  $(function() {
    return $('.flash').each(function() {
      return $(this).slideDown(300).delay(1500).slideUp(300, function() {
        return $(this).remove();
      });
    });
  });
...
(function() {
  ...
  autoComplete = function() {
    return $('.movie-title').each(function() {
      var finishLoad, load;
      $(this).autocomplete({
        source: '/searches',
        search: function() {
        ...
        response: function(event, ui) {
          return finishLoad(this);
        },
        select: function(event, ui) {
          var self;
          self = this;
          load(this);
          return $.getJSON('/searches/' + ui.item.id).done(function(data) {
            var fields;
            finishLoad(self);
            $(self).val(data.title);
            ...
            return fields.find('.poster-url').val(data.poster_url);
          });
        }
      });
      load = function(item) {
        return $(item).after("<div class='loading'></div>").nextAll('.loading').animate({
          width: '80%'
        }, 5000);
      };
      return finishLoad = function(item) {
        return $(item).nextAll('.loading').stop().animate({
          width: '100%'
        }, 100).fadeOut();
      };
    });
  ...
  $(function() {
    return $('.nav-toggle').click(function() {
      if ($('.nav-list').is(':visible')) {
        return $('.nav-list').slideUp();
      } else {
        return $('.nav-list').slideDown();
      }
    });
  });
...
(function() {
  var collapseMovieFields, makeDaysSortable, toggleAddDay;

  $(function() {
    collapseMovieFields();
    toggleAddDay($(this));
    makeDaysSortable();
    $(document).on('nested:fieldAdded:movies', function(event) {
      collapseMovieFields();
      toggleAddDay(event.field);
      return makeDaysSortable();
    });
    $('.showing-fields').on('nested:fieldAdded', function(event) {
      var day, field, label;
      day = $(event.link).data('predefined-day');
      field = event.field.find('.day-field');
      label = event.field.find('.day-label');
      $(field).val(day);
      return $(label).text(day);
    });
    $("a[rel*=leanModal]").leanModal();
    return $('.date-field').datepicker({
      prevText: "<",
      nextText: ">",
      minDate: 0,
      dateFormat: 'dd-mm-yy',
      onSelect: function(d) {
        ...
        return $(this).parent().next('.form-field').children('.date-field').datepicker("setDate", d);
      }
    });
  });

  collapseMovieFields = function() {
    var checkDelay, timeoutID;
    timeoutID = [];
    checkDelay = function(fields) {
      return timeoutID[fields.attr('id')] = setTimeout(function() {
        if (!$(fields).is(':hover')) {
          return fields.children('.collapsed').slideUp();
        }
      }, 1);
    };
    $('.movie-fields').each(function() {
      return $(this).focusin(function() {
        ...
        if (timeoutID[$(this).attr('id')]) {
          ...
          return timeoutID[$(this).attr('id')] = null;
        }
      });
    });
    return $('.movie-fields').each(function() {
      return $(this).focusout(function() {
        return checkDelay($(this));
      });
    });
  };

  toggleAddDay = function(field) {
    field.find('.add-button').click(function() {
      console.log('add clicked');
      return $(this).slideToggle(80).next('.form-field').slideToggle(80);
    });
    return field.find('.day-link').click(function() {
      console.log('link clicked');
      return $(this).parent().slideToggle(80).prev('.add-button').slideToggle(80);
    });
  };

  makeDaysSortable = function() {
    return $('.showing-fields').sortable({
      axis: 'y',
      update: function() {
        return $(this).children().each(function() {
          return $(this).find('.position-field').val($(this).index());
        });
      }
    });
  };
